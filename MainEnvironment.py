...
class MainEnvironment(gym.Env):
    ...
    def __init__(self, pt, student, ts):
        ...
        self.p_block_knowledge = set_block_knowledge(self.pt.get_p_block_dct())
        self.d_block_knowledge = set_block_knowledge(self.pt.get_d_block_dct())
        self.f_block_knowledge = set_block_knowledge(self.pt.get_f_block_dct())
        ...
        self.last_grade = grade2onehot(self.knowledge2grade())
        self.time_step = ts
        self.orig_ts = ts
    ...
    def learn(self, name, specific):
        ...
        if name != 's':
            reward += forget(self.s_block_knowledge, fr)
        if name != 'p':
            reward += forget(self.p_block_knowledge, fr)
        if name != 'd':
            reward += forget(self.d_block_knowledge, fr)
        if name != 'f':
            reward += forget(self.f_block_knowledge, fr)
    ...
    def assessment(self):
        ...
        if self.last_grade.index(1) <= self.student.goal_mark.index(1):
            ...
            print(f'Learning finished early with ts of {self.time_step}/{self.orig_ts}')
        else:
            done = False
        return reward, done

    def knowledge2grade(self):
        s_grade = block_knowledge2grade(self.s_block_knowledge)
        p_grade = block_knowledge2grade(self.p_block_knowledge)
        d_grade = block_knowledge2grade(self.d_block_knowledge)
        f_grade = block_knowledge2grade(self.f_block_knowledge)
